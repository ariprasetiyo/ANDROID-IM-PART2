...
import at.vcity.androidim.domain.realmToken;
...
public class realmTokenRealmProxy extends realmToken
    ...
    static final class realmTokenColumnInfo extends ColumnInfo {
        ...
        realmTokenColumnInfo(String path, Table table) {
            final Map<String, Long> indicesMap = new HashMap<String, Long>(3);
            this.tokenIndex = getValidColumnIndex(path, table, "realmToken", "token");
            indicesMap.put("token", this.tokenIndex);

            this.refresh_tokenIndex = getValidColumnIndex(path, table, "realmToken", "refresh_token");
            indicesMap.put("refresh_token", this.refresh_tokenIndex);

            this.bearerIndex = getValidColumnIndex(path, table, "realmToken", "bearer");
    ...
    realmTokenRealmProxy(ColumnInfo columnInfo) {
    ...
    public static Table initTable(ImplicitTransaction transaction) {
        if (!transaction.hasTable("class_realmToken")) {
            Table table = transaction.getTable("class_realmToken");
        ...
        return transaction.getTable("class_realmToken");
    }

    public static realmTokenColumnInfo validateTable(ImplicitTransaction transaction) {
        if (transaction.hasTable("class_realmToken")) {
            Table table = transaction.getTable("class_realmToken");
            ...
            final realmTokenColumnInfo columnInfo = new realmTokenColumnInfo(transaction.getPath(), table);
            ...
            throw new RealmMigrationNeededException(transaction.getPath(), "The realmToken class is missing from the schema for this Realm.");
        }
    }
    ...
    @SuppressWarnings("cast")
    public static realmToken createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        ...
        if (update) {
            ...
            if (!json.isNull("token")) {
                ...
                if (rowIndex != TableOrView.NO_MATCH) {
                    obj = new realmTokenRealmProxy(realm.schema.getColumnInfo(realmToken.class));
                    obj.realm = realm;
        ...
        if (obj == null) {
            if (json.has("token")) {
                if (json.isNull("token")) {
                    obj = realm.createObject(realmToken.class, null);
                } else {
                    obj = realm.createObject(realmToken.class, json.getString("token"));
                }
            } else {
                obj = realm.createObject(realmToken.class);
            }
        }
        if (json.has("token")) {
            if (json.isNull("token")) {
    ...
    @SuppressWarnings("cast")
    public static realmToken createUsingJsonStream(Realm realm, JsonReader reader)
        throws IOException {
        realmToken obj = realm.createObject(realmToken.class);
        reader.beginObject();
    ...
    public static realmToken copyOrUpdate(Realm realm, realmToken object, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        if (object.realm != null && object.realm.getPath().equals(realm.getPath())) {
            return object;
        }
        realmToken realmObject = null;
        ...
        if (canUpdate) {
            ...
            if (rowIndex != TableOrView.NO_MATCH) {
                realmObject = new realmTokenRealmProxy(realm.schema.getColumnInfo(realmToken.class));
    ...
    public static realmToken copy(Realm realm, realmToken newObject, boolean update, Map<RealmObject,RealmObjectProxy> cache) {
        realmToken realmObject = realm.createObject(realmToken.class, newObject.getToken());
    ...
    public static realmToken createDetachedCopy(realmToken realmObject, int currentDepth, int maxDepth, Map<RealmObject, CacheData<RealmObject>> cache) {
        if (currentDepth > maxDepth || realmObject == null) {
            return null;
        }
        CacheData<realmToken> cachedObject = (CacheData) cache.get(realmObject);
        realmToken standaloneObject;
        if (cachedObject != null) {
            ...
            standaloneObject = new realmToken();
    ...
    static realmToken update(Realm realm, realmToken realmObject, realmToken newObject, Map<RealmObject, RealmObjectProxy> cache) {
        realmObject.setRefresh_token(newObject.getRefresh_token());
        realmObject.setBearer(newObject.getBearer());
        return realmObject;
    }
    ...
    public String toString() {
        ...
        StringBuilder stringBuilder = new StringBuilder("realmToken = [");
    ...
    public boolean equals(Object o) {
        ...
        realmTokenRealmProxy arealmToken = (realmTokenRealmProxy)o;

        String path = realm.getPath();
        String otherPath = arealmToken.realm.getPath();
        if (path != null ? !path.equals(otherPath) : otherPath != null) return false;;

        String tableName = row.getTable().getName();
        String otherTableName = arealmToken.row.getTable().getName();
        if (tableName != null ? !tableName.equals(otherTableName) : otherTableName != null) return false;

        if (row.getIndex() != arealmToken.row.getIndex()) return false;
